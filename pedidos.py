...
def pedidos(container, lista_codigo_pedido, tenant_id):
    ...
    ttk.Label(container, text="Controle de pedidos", font=("Arial", 24, "bold"), background=cor_fundo, foreground=cor_texto).grid(row=1, column=0, columnspan=2, padx=10, pady=5, sticky="w")
    
    colunas = ("Nº do Pedido", "Vendedor", "Valor")
    lista_produto = ttk.Treeview(container, columns=colunas, show="headings", height=6)

    # Estilo para a Treeview (ajustando o espaçamento entre as linhas e colunas)
    style = ttk.Style()
    style.configure("Treeview", rowheight=30)  # Aumenta o espaçamento vertical
    style.configure("Treeview.Heading", padding=(10, 5))  # Aumenta o padding das cabeçalhos
    style.configure("Treeview", padding=(10, 5))  # Aumenta o padding das colunas
